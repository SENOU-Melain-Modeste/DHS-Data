...
svy: ta v613corr RealisaAttentes_corr_new if Target1 == 1 & v012 > 39, , pearson  row
...
svy: graph dot v613_corr, over(v013) by(RealisaAttentes_corr_new)
graph dot v218, over(v013) by(RealisaAttentes_corr_new)
graph  RealisaAttentes_corr_new
...
cmp (RealisaAttentes_corr_new = Education_femme Education_epoux Religion_group AgeFirstBirth_corr NumberUnion ModernContraception MediaExposure beating_justified ParticipationDecision) (Target2 = Residence v024 Education_femme Religion_group Infecund Sterelized ChildLost_Exp NeverGiveBorn), ind($cmp_mlogit $cmp_logit)

cmp (salaire = educ experience age) (travail = educ experience age), ind($cmp_cont $cmp_probit)
ind($cmp_mprobit $cmp_probit) 

****Determinant mass media

logit FP_MediaExposure age Residence AccessElectricity i.Wealth_Tercile i.SituationMatri i.Religion_group i.OccupationFemme
outreg2 using ResultMedialogit.doc,  ctitle(logit) dec(3) replace

****Determinant Contraception

logit ModernContraception age Residence FP_MediaExposure i.Wealth_Tercile i.SituationMatri i.Religion_group i.OccupationFemme
outreg2 using ResultModerncontralogit.doc,  ctitle(logit) dec(3) replace

predict resid, resid

bys RealisaAttentes: sum v218
bys RealisaAttentes: sum ideal_cluster 

**** ARTICLE 1:Unravelling factors associated with the non-compliance between desired and actual number of children among married women in Benin******. 

sum  RealisaAttentes i.Age_FirstMariage_cat i.ChildComposit Residence  ModernContraception ideal_cluster  NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux  i.Education_femme i.Education_epoux HaveSon beating_justified i.FertilityPreference i.Wealth_Tercile FP_MediaExposure i.SituationMatri decisionMaking1 AccessElectricity

****mLogit

 
mlogit RealisaAttentes2 Contraception  ideal_cluster  score FP_Discussion_Commu age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , base(2) 
outreg2 using Resultsmlogit.doc,  ctitle(Spec1) dec(3) replace

margins, dydx(*)


mlogit RealisaAttentes2  FP_MediaExposure ideal_cluster score age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , base(2) 
outreg2 using Resultsmlogit.doc,  ctitle(Spec2) dec(3) append

mlogit RealisaAttentes2 Contraception  MediaExposure ideal_cluster FP_Discussion_Commu  age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , base(2) 
outreg2 using Resultsmlogit.doc,  ctitle(Spec3) dec(3) append

****Control function
**1st step
reg Contraception age  i.Age_FirstMariage_cat i.ChildComposit Residence FP_MediaExposure visitForFP FP_Discussion_Commu i.Wealth_Tercile i.SituationMatri i.Religion_group MotherEducYear i.OccupationFemme i.FertilityPreference
outreg2 using Resultcontralogit.doc,  ctitle(probit) dec(3) replace
predict Predit_contra
kdensity Predit_contra
gen resid_contra= Contraception-Predit_contra
gen Contra_resid = Contraception*resid_contra
gen invmills2 = normalden(Predit_contra)/normal(Predit_contra)
gen lambda = cond(Contraception==1, normalden(Predit_contra)/normal(Predit_contra), - normalden(Predit_contra)/(1-normal(Predit_contra)))


reg FP_MediaExposure AccessElectricity MediaExposure age  i.Age_FirstMariage_cat i.ChildComposit Residence  visitForFP FP_Discussion_Commu i.Wealth_Tercile i.SituationMatri i.Religion_group MotherEducYear i.OccupationFemme i.FertilityPreference
outreg2 using ResultcontralogitMedia.doc,  ctitle(LPM) dec(3) replace
predict Predit_FP_Media
kdensity Predit_FP_Media
gen resid_FP_Media= FP_MediaExposure-Predit_FP_Media

reg MediaExposure AccessElectricity age  i.Age_FirstMariage_cat i.ChildComposit Residence  visitForFP FP_Discussion_Commu i.Wealth_Tercile i.SituationMatri i.Religion_group MotherEducYear i.OccupationFemme i.FertilityPreference
outreg2 using ResultcontralogitMedia.doc,  ctitle(LPM) dec(3) replace
predict Predit_Media
kdensity Predit_Media
gen resid_Media= MediaExposure-Predit_Media

*** 2nd step

bootstrap, reps(20): mlogit RealisaAttentes2 v613 Contraception  resid_contra  age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , base(2) 
margins, dydx(*)
outreg2 using ResultsCFmlogit.doc,  ctitle(Spec1) dec(3) replace


bootstrap, reps(100): mlogit RealisaAttentes MediaExposure  resid_Media  ideal_cluster age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , base(2) rrr
outreg2 using ResultsCFmlogit.doc,  ctitle(Spec2) dec(3) append


bootstrap, reps(10): mlogit RealisaAttentes2 /*Contraception  resid_contra*/ FP_MediaExposure resid_FP_Media   ideal_cluster age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , base(2) 
outreg2 using ResultsCFmlogit.doc,  ctitle(Spec3) dec(3) append




****ARTICLE 2.	What are the driving factors of overachievement fertility decision among women in Benin 

recode RealisaAttentes (1 = 3 "OVER")(2 = 2 "EXACT")(3 = 1 "UNDER"), gen(overachived)
****mprobit*************************************************************

bootstrap, reps(100):oprobit overachived Contraception  ideal_cluster age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , vce(robust)
outreg2 using ResultsOprobit.doc,  ctitle(CONTRACEPTION) dec(3) replace
margins
outreg2 using ResultsMEOprobit.doc,  ctitle(CONTRACEPTION) dec(3) replace


bootstrap, reps(100):oprobit overachived  MediaExposure ideal_cluster age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , vce(robust)
outreg2 using ResultsOprobit.doc,  ctitle(MEDIA) dec(3) append
margins
outreg2 using ResultsMEOprobit.doc,  ctitle(MEDIA) dec(3) append


bootstrap, reps(100):oprobit overachived Contraception MediaExposure ideal_cluster age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference , vce(robust)
outreg2 using ResultsOprobit.doc,  ctitle(ALL) dec(3) append
margins
outreg2 using ResultsMEOprobit.doc,  ctitle(ALL) dec(3) append

***eoprobit RealisaAttentes [indepvars], entreat(depvar_tr [= varlist_tr]) [options]
*** endogeneous Contraception
bootstrap, reps(5): eoprobit overachived  ideal_cluster age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference, entreat(Contraception = FP_MediaExposure visitForFP FP_Discussion_Commu)
outreg2 using Resultseoprobit.doc,  ctitle(eoprobit) dec(3) replace

margins, post
outreg2 using ResultsMEeoprobit.doc,  ctitle(MEeoprobit) dec(3) replace
tab RealisaAttentes


****endogeneous MediaExposure
bootstrap, reps(5): eoprobit overachived  ideal_cluster age age_sq i.Age_FirstMariage_cat_3 Residence   NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux i.Wealth_Tercile MotherEducYear HusbandEduYear i.FertilityPreference, entreat(MediaExposure = AccessElectricity)
outreg2 using Resultseoprobit_Media.doc,  ctitle(eoprobit) dec(3) replace
margins, post
outreg2 using ResultsMEeoprobit_Media.doc,  ctitle(MEeoprobit) dec(3) replace
...
poisson RealisaAttentes i.Age_FirstMariage_cat i.ChildComposit Residence  Contraception  NumberUnion DeathChild i.Religion_group i.EmploymentDuration i.OccupationFemme i.OccupationEpoux  i.Education_femme i.Education_epoux HaveSon beating_justified i.FertilityPreference i.Wealth_Tercile FP_MediaExposure i.SituationMatri decisionMaking2, vce(robust)
outreg2 using Resultspoisson.doc,  ctitle(poisson) dec(3) replace
margins, post
outreg2 using ResultsMEpoisson.doc,  ctitle(MEmprobit) dec(3) replace
